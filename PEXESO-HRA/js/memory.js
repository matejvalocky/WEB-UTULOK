...
;(function( window ) {
  ...
  Memory.prototype._setupGame = function() {
    ...
    this.gameMetaHTML = '<div class="mg__meta--left">\
      <span class="mg__meta--level">Stupeň obtiažnosti: \
      <span id="mg__meta--level">' + this.chosenLevel + '</span>\
      </span>\
      <span class="mg__meta--moves">Počet ťahov: \
      <span id="mg__meta--moves">' + this.numMoves + '</span>\
      </span>\
      </div>\
      <div class="mg__meta--right">\
      <button id="mg__button--restart" class="mg__button">Začať novú hru</button>\
      </div>';
    this.gameMeta.innerHTML = this.gameMetaHTML;
    this.game.appendChild(this.gameMeta);

    this.gameStartScreenHTML = '<h2 class="mg__start-screen--heading">Vitajte! Zahrajte si pexeso so psíkmi</h2>\
      <p class="mg__start-screen--text">Obráťte dieliky a pokúste sa ich priradiť k dvojiciam. Spárujte všetky dieliky a vyhrajte. Pokúste sa dokončiť hru v čo najmenšom počte ťahov!</p>\
      <h3 class="mg__start-screen--sub-heading">Zvoľ obtiažnosť</h3>\
      <ul class="mg__start-screen--level-select">\
      <li><span data-level="1">Obtiažnosť 1 - Ľahká (4 x 2)</span></li>\
      <li><span data-level="2">Obtiažnosť 2 - Stredná (6 x 3)</span></li>\
      <li><span data-level="3">Obtiažnosť 3 - Ťažká (8 x 4)</span></li>\
      </ul>';
  ...
  Memory.prototype._winGame = function() {
    ...
    if (this.options.onGameEnd() === false) {
      this._clearGame();
      this.gameMessages.innerHTML = '<h2 class="mg__onend--heading">Paráda!</h2>\
        <p class="mg__onend--message">Hru si dohrala v  ' + this.numMoves + ' ťahoch. Parádička!</p>\
        <button id="mg__onend--restart" class="mg__button">Zahraj si hru znovu!</button>';
